...
import { useDeleteSetoranMutation, useSetoranDetailQuery } from '@/lib/queries/setoran'
...
import { SetoranEditForm } from './[id]/edit/setoran-edit-form'
import { SetoranCreateForm } from './components/setoran-create-form'
...
function CashFlowSummary({ summary, recordCount, currentFilter }: {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-2">
      ...
      <motion.div variants={cardVariants} whileHover="hover">
        <Card className={`bg-gradient-to-br ${(stats.sisaCash ?? 0) > 0
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${(stats.sisaCash ?? 0) > 0 ? 'text-orange-700' :
                  (stats.sisaCash ?? 0) < 0 ? 'text-gray-700' : 'text-yellow-700'
                  }`}>
                  Sisa Cash di Tangan Sales
                </p>
                <p className={`text-2xl font-bold ${(stats.sisaCash ?? 0) > 0 ? 'text-orange-900' :
                ...
                <p className={`text-xs mt-1 ${(stats.sisaCash ?? 0) > 0 ? 'text-orange-600' :
              ...
              <div className={`p-3 rounded-full ${(stats.sisaCash ?? 0) > 0 ? 'bg-orange-200' :
...
export default function DepositsPage() {
  ...
  const [selectedSetoran, setSelectedSetoran] = useState<any | null>(null)
  const [detailModalOpen, setDetailModalOpen] = useState(false)
  const [editModalOpen, setEditModalOpen] = useState(false)
  const [editSubmitting, setEditSubmitting] = useState(false)
  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [createSubmitting, setCreateSubmitting] = useState(false)
  const createFormId = "setoran-create-form"

  const detailSetoranQuery = useSetoranDetailQuery(selectedSetoran?.id_setoran ?? 0)
  const detailSetoranData = (detailSetoranQuery.data as { data: any })?.data
  const modalFormId = selectedSetoran
    ? `setoran-edit-form-${selectedSetoran.id_setoran}`
    : undefined
    ...
    <TooltipProvider>
      <div className="h-full flex flex-col overflow-hidden bg-white">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ width: COLUMN_WIDTHS.penerima }} className="px-2 flex flex-col justify-center overflow-hidden">
                        ...
                        <span className={`text-xs font-medium ${runningBalance > 1000000 ? 'text-orange-600' :
                          runningBalance < 0 ? 'text-red-600' : 'text-green-600'
                          }`}>
                          Cash On Hand: {formatCurrency(Math.abs(runningBalance))}
                        </span>
                        <span className={`text-xs ${runningBalance > 1000000 ? 'text-orange-500' :
                      ...
                      <div style={{ width: COLUMN_WIDTHS.aksi }} className="px-2 flex items-center gap-0.5">
                        {item.event_type === 'SETORAN' ? (
                          ...
                          <span className="text-xs text-gray-400 px-1" title="Hanya transaksi setoran yang dapat diedit">
                            -
                          </span>
                        )}
                      </div>
        ...
        <ModalBox
          ...
          description="Catat setoran cash baru"
          footer={
        ...
        {/* Detail Modal */}
        <ModalBox
          open={detailModalOpen && !!selectedSetoran}
          onOpenChange={(open) => {
          ...
          title={
            selectedSetoran
              ? `Detail Setoran #${selectedSetoran.id_setoran}`
              : 'Detail Setoran'
          }
          description={
            selectedSetoran?.tanggal_setoran
              ? `Tanggal ${selectedSetoran.tanggal_setoran}`
              : undefined
          }
          className="max-w-lg !h-[40vh]"
        >
          {selectedSetoran && (
            <SetoranCreateForm
              mode="detail"
              initialData={{
                id_setoran: selectedSetoran.id_setoran,
                total_setoran: selectedSetoran.total_setoran,
                penerima_setoran: selectedSetoran.penerima_setoran,
              }}
            />
          )}
        </ModalBox>
        ...
        {/* Edit Modal */}
        <ModalBox
          open={editModalOpen && !!selectedSetoran}
          onOpenChange={(open) => {
          ...
          title={
            selectedSetoran
              ? `Edit Setoran #${selectedSetoran.id_setoran}`
              : 'Edit Setoran'
          }
          description={
            selectedSetoran?.tanggal_setoran
